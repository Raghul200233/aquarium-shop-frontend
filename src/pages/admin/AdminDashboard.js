...
const AdminDashboard = () => {
  ...
  const fetchPendingOrders = async () => {
    try {
      ...
      const pendingOrdersCount = response.data.orders?.length || 0;
      
      // Create notification for new pending orders
      const newPendingOrders = response.data.orders?.filter(order => {
        const orderTime = new Date(order.createdAt);
        const now = new Date();
        const minutesAgo = (now - orderTime) / (1000 * 60);
        return minutesAgo < 30; // Orders from last 30 minutes
      }) || [];

      if (newPendingOrders.length > 0 && stats.pendingOrders !== pendingOrdersCount) {
        toast.success(`📦 ${newPendingOrders.length} new order${newPendingOrders.length > 1 ? 's' : ''} pending!`, {
          duration: 5000,
          icon: '🛒'
        });
      }
              ...
              <div className="notification-list">
                {stats.pendingOrders > 0 ? (
                  <>
                    <div className="notification-item urgent">
                      ...
                      <Link to="/admin/orders" className="notification-link">View →</Link>
                    </div>
                ...
                {stats.lowStockProducts > 0 && (
                  <div className="notification-item warning">
                    ...
                    <Link to="/admin/products?filter=lowstock" className="notification-link">Check →</Link>
                  </div>
                )}
              </div>
            </div>
        ...
        {stats.lowStockProducts > 0 && (
          <div className="alert-card warning">
            ...
            <div className="alert-content">
              ...
              <p>You have {stats.lowStockProducts} product{stats.lowStockProducts > 1 ? 's' : ''} running low on stock.</p>
